...
angles = [0, 10, 20, 30, 40];  % angles
hu_all = zeros(length(angles), 7);


figure('Name', 'Shear Views');
for i = 1:length(angles)
    angle = angles(i);
    shx = tand(angle);  

    % shear
    T = affine2d([1 shx 0; 0 1 0; 0 0 1]);
    warped = imwarp(bw, T, 'OutputView', imref2d(size(bw)));

    subplot(2, ceil(length(angles)/2), i);
    imshow(warped); title(sprintf('Shear %d°', angle));

    hu = computeHuMoments(warped);
    hu_all(i,:) = hu;
end


disp('angles vs Hu moments:');
disp(array2table(hu_all, ...
    'VariableNames', {'Hu1','Hu2','Hu3','Hu4','Hu5','Hu6','Hu7'}, ...
    'RowNames', strcat(string(angles), '°')));
